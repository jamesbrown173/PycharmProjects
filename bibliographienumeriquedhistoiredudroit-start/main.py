...
def make_request_with_proxy(url):
    try:
        response = requests.get(url, proxies={'http': proxy, 'https': proxy}, timeout=None)
        return response
...
base_url = "https://bibliographienumeriquedhistoiredudroit-ifg.univ-lorraine.fr/s/droit/item/"
all_data = []
...
with open('output.csv', 'a', newline='', encoding='utf-8-sig') as csvfile:
    ...
    csv_writer.writerow(["URL", "Titre court", "Résumé", "Lieu", "ISSN", "Edition", "Editeur", "Description", "A une parte",
                        "Titre", "Auteur", "Année", "Type", "Pages", "ISBN", "Titre du périodique", "Numéro", "Mot-clé", "Thesaurus"])
    ...
    for item_number in range(24573,999999):  # You can adjust the range as needed
        ...
        if h4_title_10:
            thesaurus_names = [sibling.get_text() for sibling in h4_title_10.find_next_siblings()]
            page_profile_dict["Thesaurus_names"] = thesaurus_names
            thesaurus_links = ["https://bibliographienumeriquedhistoiredudroit-ifg.univ-lorraine.fr/" + sibling2.find('a', href=True)['href'] for sibling2 in h4_title_10.find_next_siblings()]
        ...
        csv_writer.writerow(
            [page_profile_dict.get(key, "None") for key in ["URL", "Titre court", "Résumé", "Lieu", "ISSN",
                                                            "Edition", "Editeur", "Description", "A une parte",
                                                            "Titre", "Auteur", "Année", "Type", "Pages",
                                                            "ISBN", "Titre du périodique", "Numéro", "Mot-clé",
                                                            "Thesaurus"]])
